...
class Auth {
    ...
    public function handle_cognito_callback() {
        ...
        try {
            ...
            if (isset($state_parts[1]) && !empty($state_parts[1])) {
                ...
                if (!empty($decoded_redirect) && filter_var($decoded_redirect, FILTER_VALIDATE_URL)) {
    ...
    public function maybe_redirect_to_cognito() {
        ...
        $allowed_actions = ['rp', 'resetpass', 'lostpassword', 'retrievepassword'];
    ...
    public function handle_logged_in_user_on_login_page() {
        ...
        // Don't redirect if this is a password reset or other special action
        $allowed_actions = ['rp', 'resetpass', 'lostpassword', 'retrievepassword', 'register'];
        if (isset($_GET['action']) && in_array($_GET['action'], $allowed_actions)) {
            return;
        }
        ...
        if (isset($_GET['redirect_to']) && !empty($_GET['redirect_to'])) {
            $redirect_to = $_GET['redirect_to'];
        } elseif (user_can($current_user->ID, 'edit_posts') || user_can($current_user->ID, 'manage_options')) {
            // Admin users go to dashboard
            $redirect_to = admin_url();
        } else {
            // Regular users go to homepage
    ...
    private function sync_user_groups_from_cognito($user_id, $cognito_data) {
        $cognito_groups = isset($cognito_data['cognito:groups']) ? $cognito_data['cognito:groups'] : [];
        ...
        <div class="cognito-login-section">
            <p style="text-align: center; margin: 20px 0;">
                <a href="<?php echo esc_url($login_url); ?>" 
                   ...
                   style="width: 100%; text-align: center; background-color: #ff9900; border-color: #ff9900; color: white; text-decoration: none; display: inline-block; padding: 10px;">
                    <?php _e('Login with Cognito', 'wp-cognito-auth'); ?>
                </a>
            </p>
    ...
    private function get_cognito_jwks() {
        ...
        $response = wp_remote_get($jwks_url, ['timeout' => 15]);
        if (is_wp_error($response)) {
            return false;
        }
