<?php
/**
 * Auth class for handling Cognito authentication.
 *
...
/**
 * Class Auth - Handles Cognito authentication and login flows.
 */
class Auth {
	/**
		...
		try {
			...
			if ( isset( $state_parts[1] ) && ! empty( $state_parts[1] ) ) {
				...
				if ( ! empty( $decoded_redirect ) ) {
					// Security: Ensure redirect is safe (same site or relative URL).
					$site_url = site_url();
					if ( 0 === strpos( $decoded_redirect, '/' ) ) {
						// Relative URL is safe.
						$redirect_to = $decoded_redirect;
					} elseif ( filter_var( $decoded_redirect, FILTER_VALIDATE_URL ) && 0 === strpos( $decoded_redirect, $site_url ) ) {
						// Absolute URL to same site is safe.
						$redirect_to = $decoded_redirect;
					}
				}
			}
			...
			if ( empty( $redirect_to ) || home_url() === $redirect_to ) {
	...
	public function handle_logout() {
		...
		if ( isset( $_GET['redirect_to'] ) ) {
			...
			// Security: Allow redirects to the same site or relative URLs.
			$site_url = site_url();
			if ( 0 === strpos( $requested_redirect, $site_url ) || 0 === strpos( $requested_redirect, '/' ) ) {
	...
	public function maybe_redirect_to_cognito() {
		...
		if ( isset( $_GET['redirect_to'] ) ) {
			$requested_redirect = sanitize_url( wp_unslash( $_GET['redirect_to'] ) );
			// Security: Only allow redirects to the same site.
			if ( 0 === strpos( $requested_redirect, $site_url ) ) {
				$redirect_to = $requested_redirect;
			}
		} elseif ( isset( $_POST['redirect_to'] ) ) {
			$requested_redirect = sanitize_url( wp_unslash( $_POST['redirect_to'] ) );
			// Security: Only allow redirects to the same site.
			if ( 0 === strpos( $requested_redirect, $site_url ) ) {
				$redirect_to = $requested_redirect;
			}
			...
			$referer = sanitize_url( wp_unslash( $_SERVER['HTTP_REFERER'] ) );
			if ( 0 === strpos( $referer, $site_url ) && false === strpos( $referer, 'wp-login.php' ) ) {
				$redirect_to = $referer;
			}
		}
	...
	public function add_cognito_login_button() {
		...
		$button_bg_color   = get_option( 'wp_cognito_auth_button_bg_color', '#ff9900' );
		$button_text_color = get_option( 'wp_cognito_auth_button_text_color', '#ffffff' );
		$button_text       = get_option( 'wp_cognito_auth_button_text', __( 'Login with Cognito', 'wp-cognito-auth' ) );

		// Calculate hover color (darker version of background color)
		$hover_color = $this->darken_color( $button_bg_color, 20 );

		// Preserve redirect_to parameter if present and safe
		if ( isset( $_GET['redirect_to'] ) ) {
			$requested_redirect = sanitize_url( wp_unslash( $_GET['redirect_to'] ) );
			// Security: Only preserve redirects to the same site or allow relative URLs
			$site_url = site_url();
			if ( 0 === strpos( $requested_redirect, $site_url ) || 0 === strpos( $requested_redirect, '/' ) ) {
				$login_url = add_query_arg( 'redirect_to', urlencode( $requested_redirect ), $login_url );
			}
		}
		?>
		<div class="cognito-login-section">
			<p style="text-align: center; margin: 20px 0;">
				<a href="<?php echo esc_url( $login_url ); ?>"
					...
					style="width: 100%; text-align: center; background-color: <?php echo esc_attr( $button_bg_color ); ?>; border-color: <?php echo esc_attr( $button_bg_color ); ?>; color: <?php echo esc_attr( $button_text_color ); ?>; text-decoration: none; display: inline-block; padding: 10px;">
	...
	public function add_cognito_login_fallback() {
		...
		if ( isset( $_GET['redirect_to'] ) ) {
			$requested_redirect = sanitize_url( wp_unslash( $_GET['redirect_to'] ) );
			// Security: Only preserve redirects to the same site.
			$site_url = site_url();
			if ( 0 === strpos( $requested_redirect, $site_url ) ) {
				$login_url = add_query_arg( 'redirect_to', urlencode( $requested_redirect ), $login_url );
			}
		...
		jQuery(document).ready(function($) {
			...
			if ($form.length > 0) {
				...
				var cognitoButton = '<div class="cognito-login-section" style="margin: 20px 0;">' +
					...
					'style="width: 100%; text-align: center; background-color: <?php echo esc_js( $button_bg_color ); ?>; border-color: <?php echo esc_js( $button_bg_color ); ?>; color: <?php echo esc_js( $button_text_color ); ?>; text-decoration: none; display: inline-block; padding: 10px;">' +
				...
				$('.wp-cognito-login').hover(
					...
					function() {
						$(this).css({
							'background-color': '<?php echo esc_js( $button_bg_color ); ?>',
							'border-color': '<?php echo esc_js( $button_bg_color ); ?>'
						});
					}
				);
			}
